...
const DraggableMap: FC<{ height: number }> = ({ height }) => {
      ...
      if (isFocused) {
        ...
        if (status === "granted") {
          try {
            ...
            handleResionChangeComplete(newRegion);
          } catch (error) {
  ...
  // }, [location]);
  //end
  const handleResionChangeComplete = async (newRegion: Region) => {
  ...
  return (
    <View style={{ height: height, width: "100%" }}>
      <MapView
        ...
        minZoomLevel={12}
        pitchEnabled={false}
        onRegionChangeComplete={handleResionChangeComplete}
        style={{ flex: 1 }}
